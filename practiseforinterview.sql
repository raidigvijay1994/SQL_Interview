...
/* WRITE A SQL CODE TO FIND DIFFERENCE IN THE SALARY AND PERCENTAGE DIFFERENCE BETWEEN PREVIOUS AND CURRENT SALARY */

WITH CTE AS(
SELECT *, LAG(SALARY) OVER (ORDER BY SALARY) AS NEW_SALARY,
SALARY - LAG(SALARY) OVER (ORDER BY SALARY) AS DIFFERENCE,
100 * (SALARY - LAG(SALARY) OVER (ORDER BY SALARY))/  LAG(SALARY) OVER (ORDER BY SALARY)  AS PERCENTAGE
FROM WORKER)
...
WITH CTE AS(
SELECT *, DENSE_RANK() OVER (PARTITION BY DEPARTMENT ORDER BY SALARY DESC) AS RANKED_SALARY
FROM WORKER)
